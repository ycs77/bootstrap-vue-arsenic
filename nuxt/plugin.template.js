...
options.componentPlugins.reduce((acc, p) => (acc += `import ${p[0]} from 'bootstrap-vue-arsenic/${options.dist}/components/${p[1]}'\n` ), '') %>
// <%=
// options.directivePlugins.reduce((acc, p) => (acc += `import ${p[0]} from 'bootstrap-vue-arsenic/es/directives/${p[1]}'\n` ), '') %>

<% if (options.config) { %>
import BAConfigPlugin from 'bootstrap-vue-arsenic/<%= options.dist %>/ba-config'
...
<% } else { %>
import BootstrapVue from 'bootstrap-vue-arsenic/<%= options.dist %>'
