  ...
  <b-container tag="main">
    ...
    <div class="bd-content mb-4">
      ...
      <p class="bd-lead">
        ...
        section for more information about available components and usage.
      </p>
    </div>
    ...
    <b-row>
      <b-col class="mb-2 mb-md-0">
        <!-- Loading indicator -->
        <b-alert v-if="loading" variant="info" class="text-center" show>
          Loading JavaScript compiler...
        </b-alert>

        <!-- Reset action -->
        <b-btn v-else size="sm" variant="danger" :disabled="isDefault" @click="reset">
          Reset to default
        </b-btn>
      </b-col>

      <!-- Export actions -->
      <b-col v-if="!loading" md="auto" class="mt-2 mt-md-0">
    ...
    <!-- Editors -->
    <transition-group v-if="!loading" tag="div" class="row" name="flip">
      <!-- Left/Top column -->
        ...
        <b-row>
          ...
          <b-col cols="12" class="mt-3">
            ...
            <b-card>
              <div slot="header" class="d-flex justify-content-between align-items-center">
                <span>Console</span>
                <b-btn v-if="messages.length" size="sm" variant="outline-danger" @click="clear">
                  <span>Clear</span>
                </b-btn>
              </div>
              ...
              >
                <li v-if="!messages.length" key="empty-console" class="list-group-item">
                  &nbsp;
                </li>
                <li
...
import {
  version as bootstrapVueArsenicVersion,
  bootstrapVueVersion,
  bootstrapVersion,
  vueVersion
} from '~/content'
...
const DEFAULT_HTML = `<div>
  <h4>Hello {{ name }}!</h4>
  <p>
    <b-button size="sm" @click="toggleSize">
      Change to {{ isBigFont ? 'big' : 'small' }} font Avatar
    </b-button>
  </p>
  <b-avatar :font-size="fontSize">A</b-avatar>
</div>`

const DEFAULT_JS = `{
  data () {
    return {
      name: 'BootstrapVueArsenic',
      isBigFont: false,
      fontSize: '1rem'
    }
  },
  watch: {
    show (newVal, oldVal) {
      console.log('Avatar font size is ' + (this.isBigFont ? 'big' : 'small'))
    }
  },
  methods: {
    toggleSize () {
...
export default {
  ...
  head() {
    const title = `${this.title} | BootstrapVueArsenic`
    const description = 'Interactively play and test BootstrapVueArsenic components online.'
    return {
      title,
      meta: [
        {
  ...
  computed: {
    ...
    exportData() {
      ...
      return {
        ...
        externalCss: [
          ...
          `//unpkg.com/bootstrap-vue-arsenic@${bootstrapVueArsenicVersion}/dist/bootstrap-vue-arsenic.css`
        ],
        externalJs: [
          ...
          `//unpkg.com/bootstrap-vue-arsenic@${bootstrapVueArsenicVersion}/dist/bootstrap-vue-arsenic.js`
        ]
      }
    },
    codepenData() {
    ...
    codesandboxData() {
      ...
      const dependencies = {
        ...
        'bootstrap-vue-arsenic': bootstrapVueArsenicVersion,
        vue: vueVersion
      }
